...
use crate::session::observation::{
    CurrentRevisionContext, RevisionScope, RevisionSelection, required_title,
    resolve_revision_for_write, staged_body, validated_track_id,
};
...
pub fn open_input_request(options: InputRequestOpenOptions) -> Result<InputRequestOpenResult> {
    ...
    let mut event = ShoreEvent::new(
        ...
        InputRequestOpenedPayload {
            ...
            target: target.clone(),
            // `pointbreak input-request open` only opens review-domain requests.
            task_target: None,
            reason_code,
            title,
            body,
