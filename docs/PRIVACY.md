...
Pointbreak includes **optional** telemetry that is **disabled by default**. You must explicitly opt-in during first run or via command:

```bash
pointbreak telemetry enable
```
...
```bash
pointbreak telemetry disable
pointbreak telemetry status    # Check current status
```
...
When using Pointbreak with AI assistants (Claude, etc.), your **conversations with the AI** may include information about your debugging session. This data is sent to the AI service provider, not to us.
...
Pointbreak connects to your IDE's Debug Adapter Protocol (DAP) interface. All communication happens locally on your machine. No data is sent to IDE vendors (Microsoft, Cursor, etc.) by Pointbreak.
...
- ✅ Optional telemetry is anonymous and opt-in
