...
| Follow-up missing from Change attention | This walkthrough's release follow-up is an operative gate by design: open a related operative request and link it with `--related-input-request`. An advisory request never changes a Change's lifecycle, so it appears only as an item in `pointbreak attention list` |
