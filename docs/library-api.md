...
[storage-model.md](storage-model.md#source-retirement)): it deletes only verified files and never
deletes recursively. A busy source is reported the same way the CLI reports it, as an error whose text
