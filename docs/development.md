...
| Developer-only LMDB proof surface | Focused closure/core tests plus `cargo bench --locked --features bench,lmdb-proof --bench store_foundation -- --lmdb-proof-open-close` and `--lmdb-smoke` | Compile every `.github/binary-targets.json` target; run plain open/close and native dependency inspection on representative macOS, Linux glibc, Linux musl, and Windows hosts | Exact reviewed sources compile and link statically; semantic smoke is non-timing and disposable; no encryption, production routing, or default-package/release inclusion |
