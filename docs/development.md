...
| Derived Change diagnostic orchestration | `just derived-change-diagnostic-selftest`, focused Rust diagnostic-boundary tests, and the affected Inspector lifecycle test | Run `just derived-change-diagnostic run-host <absolute-config>` only from a clean signed candidate with exact prebuilt product/harness/control identities and the campaign-bound platform program paths, file hashes, and required dependency-tree hashes; collect every required native platform fragment before `merge` | Independent compile, policy, native, focused browser, APFS, and real-NTFS cases complete from isolated public roots; preflight rejects substituted or unloadable tools before expensive lanes, postflight rejects file or dependency-tree drift, and one deterministic `admissible: false` report is Green only when every required case passes and remains unconditionally rejected by terminal evidence consumers |
...
| `just derived-change-diagnostic …` | Caller-selected external roots whose final component is `derived-change-diagnostic` | Requires an exact clean signed source plus exact product/harness/control and public-fixture identities. Host fragments are internal campaign transport; the sole aggregate document is `pointbreak.derived-change-diagnostic-report.v1`, always `admissible: false`, and never terminal evidence. |
