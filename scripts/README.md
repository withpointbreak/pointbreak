...
| `derived-change-diagnostic-browser.sh` and `.mjs` | Internal focused real-browser collector called by the campaign | One public L2 fixture below the case scratch root, plus an Inspector process, browser session, and retained logs/screenshots below the case evidence root | Repeated narrow exact-detail to wide split-pane transitions settle with exact-action focus and all five ordinary surfaces non-inert; every iteration and runtime error channel is retained | Dirty/wrong source or binary, missing browser prerequisite, fixture/startup failure, transition failure, runtime page/console error, or missing retained artifact |
