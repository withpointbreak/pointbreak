...
| `change-inspector-browser-manifest.mjs` | Final completion step inside the shell verifier and harness self-test | Atomically renames the already-written same-directory manifest candidate only | Exact passing browser report has zero failures and matching assertion/screenshot counts before `manifest.json` appears | Invalid/failed browser report, mismatched counts, existing completion marker, or cross-directory publication attempt |
