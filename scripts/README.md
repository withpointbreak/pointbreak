...
| `derived-change-diagnostic-campaign.mjs` | `just derived-change-diagnostic <create-authority\|shakedown-host\|run-host\|unavailable-host\|merge> <absolute-config>` | Creates caller-selected, initially empty external evidence and scratch roots; `shakedown-host` consumes a dedicated disposable host config and writes only `readiness.json` plus logs, host runs isolate each mutable case below scratch, and the merger writes `derived-change-diagnostic-report.json` once | One exact campaign binds clean signed source and its allowed-signers bytes, product/harness/control binaries, public fixture authority, APFS/NTFS hosts, the complete closed platform program inventory by absolute path and SHA-256, and the discrete Git, Rust, native, JavaScript, and browser dependency trees those programs load. The readiness command reuses the final request/configuration to verify bindings, host identity, in-place OS substrate, exact launcher loadability, topology materialization, and the topology Change-read flow before expensive lanes. Full-run preflight verifies the same bound signer authority and signature helper, sterile native compile/link chain, and direct Bash/shell/Node/Vitest/Playwright/browser launch chain; the final always-attempt postflight rehashes every program, signer file, and bound tree. Every recoverable campaign result is retained in one `admissible: false` report. | Unsafe or reused readiness roots; relocated or unloadable host substrate; global identity or root invalidity; substituted tool; dependency-tree, native resource, or signer-authority drift; lane prerequisite or case failure; required host unavailability; incomplete inventory; changed retained bytes; or a Red aggregate verdict |
| `derived-change-diagnostic.mjs`, `derived-change-diagnostic-fixture.mjs`, and `derived-change-diagnostic-report.mjs` | Internal modules exercised by `just derived-change-diagnostic-selftest` | External per-case scratch plus separate retained diagnostic roots only; no owner store and no terminal receipt/package/manifest output. Each command receives fresh scratch-scoped `CARGO_HOME`, `RUSTUP_HOME`, and `CARGO_TARGET_DIR`; ambient compiler, linker, Git, shell, Node, npm, and Playwright resolution is removed, while platform cases receive only their exact campaign-bound programs and native resource paths. | Commands continue independent roots, fail closed on unknown diagnostic children, normalize the stable public topology checkpoint while retaining raw observations, verify normalized real-path containment before hashing shared dependency trees once, inventory logs/artifacts by SHA-256, finalize campaign-bound host fragments, and merge exactly one deterministic non-admissible report | Unsafe or non-empty roots, wrong source/binary/fixture/tool identity, invalid dependency graph, corrupt collection, artifact drift, topology checkpoint drift, or report recomputation mismatch |
