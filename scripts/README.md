...
`change-inspector-browser-verify.sh --shakedown` is the bounded readiness tier for the full Change Inspector browser gate. It accepts no caller evidence root: the harness creates one temporary root, uses the same public fixture materializer, primary Inspector server, injected browser program, and launch path as the full gate, and runs one named exact-reading case. That case observes two completion-anchored quiet polls, permits only serialized `/api/v2/profile` probes, and verifies that an open disclosure plus the detail scroll remain on the same DOM nodes. A passing shakedown closes the browser, stops its servers, removes its root, prints a compact cleanup receipt, and retains no manifest or browser evidence. Setup, browser, product, or cleanup failures are non-full-attempt failures and may leave diagnostics only when cleanup itself cannot complete; do not supply or touch the reserved full-gate evidence root.
