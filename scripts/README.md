...
| `derived-change-diagnostic-campaign.mjs` | `just derived-change-diagnostic <create-authority\|run-host\|unavailable-host\|merge> <absolute-config>` | Creates only caller-selected, initially empty external roots; host runs use isolated case roots and the merger writes `derived-change-diagnostic-report.json` once | One exact campaign binds clean signed source, product/harness/control binaries, public fixture authority, APFS/NTFS hosts, explicit dependencies, all recoverable case results, and a final `admissible: false` report | Global identity or root invalidity, lane prerequisite failure, case failure, required host unavailability, incomplete inventory, changed retained bytes, or a Red aggregate verdict |
| `derived-change-diagnostic.mjs` and `derived-change-diagnostic-report.mjs` | Internal modules exercised by `just derived-change-diagnostic-selftest` | External diagnostic case roots only when called by the campaign; no owner store and no terminal receipt/package/manifest output | Commands continue independent roots, retain logs/artifacts by SHA-256, finalize campaign-bound host fragments, and merge exactly one deterministic non-admissible report | Unsafe or non-empty roots, wrong source/binary/fixture identity, invalid dependency graph, corrupt collection, artifact drift, or report recomputation mismatch |
| `derived-change-diagnostic-native.mjs` | Internal native collector called by the campaign | Fresh D0-128/L1/L7 and lifecycle roots below one host case root | Every independent native tier and all lifecycle vectors return one internal case collection; raw terminal receipts are never serialized or retained | Invalid exact-source request, native setup failure, unstable admitted root, lifecycle setup failure, or individual vector failure |
| `derived-change-diagnostic-change-read.mjs` | Internal public-fixture collector called by the campaign | Nine independent disposable Change fixture templates and workspaces below one host case root | Exact named read, control, concurrency, append/restart, fault, and storage cases return one internal collection while identity postflight remains independently observable | Fixture authority or tool drift, materialization failure, invalid collector output, failed read/control/storage case, or postflight identity change |
| `derived-change-diagnostic-browser.sh` and `.mjs` | Internal focused real-browser collector called by the campaign | One public L2 fixture, Inspector process, browser session, logs, and screenshots below one host case root | Repeated narrow exact-detail to wide split-pane transitions settle with exact-action focus and all five ordinary surfaces non-inert; every iteration and runtime error channel is retained | Dirty/wrong source or binary, missing browser prerequisite, fixture/startup failure, transition failure, runtime page/console error, or missing retained artifact |
