...
| `change-inspector-browser-verify.sh` | `POINTBREAK_BINARY=<absolute-exact-binary> just change-inspector-browser-verify <empty-root>` | Creates only a public L2 Change matrix, disposable home, retained recovery copy for the intentionally missing artifact, browser screenshots, logs, and completion-last manifest below the supplied empty root | The injected binary attests the clean source commit; 363+ bounded Change list, exact final topology and wide/narrow shared-membership fixtures, resource filters, exact Revision, distinct removed and recoverably missing resources with no substituted diff, wide/narrow light/dark density states, keyboard/focus/modal behavior, reading return path, URL-preserving clear reset, unchanged-generation DOM retention, and reduced motion pass in a real browser. The manifest is written only after its browser-reported screenshot count matches preserved files. | Non-empty/unsafe root, binary/source mismatch, fixture construction or missing-artifact containment, Inspector startup, browser environment, interaction/focus/route/layout assertion, console, or product behavior failure |
| `change-inspector-browser-verify.mjs` | Internal template consumed by the Change-first browser verifier | Browser page state and configured screenshot directory only | The injected route, viewport, preference, keyboard, modal focus, reading, and motion assertions complete without console or page errors | Change-first Inspector rendering, accessibility, navigation, or responsive behavior failed; do not invoke this template directly |
