...
const fixture=request.readRequest.fixture;
const typed=new Set(["duplicate-conflict-v1","missing-carrier-v1","mutated-carrier-v1","wrong-family-carrier-v1"]);
const sourceFailure=process.argv.includes("--test-source-preflight-failure");
const witnessFailure=process.argv.includes("--test-witness-failure");
const readyWitnessFailure=process.argv.includes("--test-ready-witness-failure");
const typedWitnessFailure=process.argv.includes("--test-typed-witness-failure");
const caseStatus=sourceFailure?{status:"skipped",failureDetail:"synthetic source preflight failure"}:{status:"passed"};
const strictWitness={oracle:"strict_parity",derived:{httpStatus:200,code:null,normalizedDocumentSha256:"${digest("1")}"},strict:{httpStatus:200,code:null,normalizedDocumentSha256:"${digest("2")}"},expectedHttpStatus:200,expectedCode:null};
const readyWitness={...strictWitness,oracle:"ready_profile_parity"};
const typedWitness={oracle:"typed_failure",observed:{schema:"pointbreak.inspect-change-projection-error",version:1,code:"projection_invalid",retryable:null,keySet:["code","message","schema","version"],canonicalSha256:"${digest("3")}"},expected:{schema:"pointbreak.inspect-change-projection-error",version:1,code:"projection_invalid",retryable:null,keySet:["code","message","schema","version"],canonicalSha256:"${digest("4")}"}};
const failureWitness=typedWitnessFailure?typedWitness:readyWitnessFailure?readyWitness:strictWitness;
const rows=(fixture === "topology-v1" ? ${JSON.stringify(readCases)} : typed.has(fixture) ? ${JSON.stringify(["profile", "changes_bare", "changes_bounded", "attention_bare", "attention_bounded", "summary_query"])} : ${JSON.stringify(readCases.slice(0, 8))}).map((caseName,index)=>(witnessFailure||readyWitnessFailure||typedWitnessFailure)&&index===0?{case:caseName,status:"failed",failureDetail:"bounded mismatch",failureWitness}:{case:caseName,...caseStatus});
...
test("promotes a ready-Profile mismatch through the semantic witness boundary", async () => {
	const input = await fixture();
	input.config.harness.argsPrefix = ["--test-ready-witness-failure"];
	const result = await runDerivedChangeChangeReadDiagnostic(input.config);
	const actual = result.cases.find(
		({ id }) => id === "mutated-carrier-v1.read.profile",
	).actual;
	assert.deepEqual(actual, {
		oracle: "ready_profile_parity",
		derived: {
			httpStatus: 200,
			code: null,
			normalizedDocumentSha256: digest("1"),
