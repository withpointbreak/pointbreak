...
use pointbreak::bench_support::derived_access::{
    ...
    QUALIFICATION_DERIVED_ACCESS_PHASE_MODE_V1, QUALIFICATION_DERIVED_ACCESS_PHASE_VERIFY_MODE_V1,
    ...
    QUALIFICATION_DERIVED_CHANGE_FIXTURE_MODE_V1, QUALIFICATION_DERIVED_CHANGE_READ_MODE_V1,
    QualificationDerivedAccessTierV1, assemble_qualification_derived_access_package_v1,
    ...
    run_qualification_derived_access_phase_v1, run_qualification_derived_access_resource_child_v1,
    ...
    run_qualification_derived_change_read_v1, verify_authority_stamp_native_receipts_v1,
    verify_qualification_derived_access_package_v1, verify_qualification_derived_access_phase_v1,
};
...
fn main() -> ExitCode {
    ...
    if arguments
        .iter()
        .any(|argument| argument == QUALIFICATION_DERIVED_CHANGE_FIXTURE_MODE_V1)
    {
        if derived_requests.len() != 1 {
            eprintln!("derived Change fixture materialization requires exactly one typed request");
            return ExitCode::from(2);
        }
        return report_derived_access_smoke(
            materialize_qualification_derived_change_fixture_from_request_v1(std::path::Path::new(
                derived_requests[0],
            )),
        );
    }
    if arguments
        .iter()
        .any(|argument| argument == QUALIFICATION_DERIVED_CHANGE_READ_MODE_V1)
    {
        if derived_requests.len() != 1 {
            eprintln!("derived Change read evidence requires exactly one typed request");
            return ExitCode::from(2);
        }
        return report_derived_access_smoke(run_qualification_derived_change_read_v1(
    ...
    {
        if derived_requests.len() != 1 {
            eprintln!("derived-access phase evidence requires exactly one typed request");
            return ExitCode::from(2);
        }
        return report_derived_access_smoke(run_qualification_derived_access_phase_v1(
            std::path::Path::new(derived_requests[0]),
        ));
    }
    if arguments
        .iter()
        .any(|argument| argument == QUALIFICATION_DERIVED_ACCESS_PHASE_VERIFY_MODE_V1)
    {
        let inputs = arguments
            .iter()
            .filter_map(|argument| argument.strip_prefix("--derived-access-input="))
            .collect::<Vec<_>>();
        if derived_requests.len() != 1 || inputs.len() != 1 {
            eprintln!(
                "derived-access phase verification requires one typed request and one bundle input"
            );
            return ExitCode::from(2);
        }
        return report_derived_access_smoke(verify_qualification_derived_access_phase_v1(
            std::path::Path::new(derived_requests[0]),
